      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          ...
          child: StatefulBuilder(
            // Сохранено: StatefulBuilder для локального state — обновляет радио-кнопки. Меняет: Local selected обновляется onTap GestureDetector, Radio for display only (no groupValue). Не удаляет: Логику — Navigator.pop(selected) возвращает значение, основной setState применяет _selectedYear = selected, _saveFilters сохраняет в prefs. Функциональность: Tap ListTile '2023' фильтрует посевы, _buildAgeDistributionChart обновляется без конфликтов — динамика ListView.builder сохраняется (itemCount: years.length + 1).
                ...
                itemBuilder: (ctx, i) {
                  ...
                  if (i == 0) {
                    ...
                    return GestureDetector(
                      // Сохранено: GestureDetector on ListTile for tap — M3-совместимо, заменяет onChanged (deprecation). Меняет: Tap sets selected, visual via InkWell ripple. Не удаляет: title/value — логика Navigator.pop(selected).
                      onTap: () {
    ...
    final selected = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: SizedBox(
          width: 300,
          child: StatefulBuilder(
            // Сохранено: StatefulBuilder для локального state — обновляет радио-кнопки. Меняет: Local selected обновляется onTap GestureDetector, Radio for display only (no groupValue). Не удаляет: Логику — Navigator.pop(selected) возвращает значение, основной setState применяет _selectedCategory = selected, _saveFilters сохраняет в prefs. Функциональность: Tap ListTile 'sown' фильтрует посевы, _buildCategoryChart обновляется без конфликтов — графики pie_chart показывают категории.
            builder: (ctx, setDialogState) {
              // Сохранено: Local setDialogState — эффективнее для диалога.
              String? selected =
