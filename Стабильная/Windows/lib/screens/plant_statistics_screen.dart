...
// === График с тремя линиями: всхожесть, погибшие, живые ===
  ...
  Widget _buildFloweringTab() {
    // Что меняет: Добавляет SingleChildScrollView вокруг Column, чтобы контент (календарь + график + кнопка) мог скроллиться при переполнении. Не удаляет: События цветения (bloomed/wilted) отображаются как раньше, график длительности периодов работает.
    return Padding(
      padding: const EdgeInsets.all(16.0),
      child: SingleChildScrollView(
        // Новый: Оборачивает Column для скролла, если высота > экрана (исправляет overflow на 179 пикселей).
        child: Column(
          children: [
  ...
  void _clearFloweringData() {
    // Что меняет: Добавляет if (mounted) перед setState(), чтобы не обновлять UI после dispose() экрана (исправляет "disposed RenderObject mutated"). Не удаляет: Очистка данных цветения в провайдере работает, UI обновится только если экран открыт.
    final plantProvider = Provider.of<PlantProvider>(context, listen: false);
    plantProvider.clearFloweringData(widget.plant.permanentId);
    if (mounted) {
      // Новый: Проверяет, что экран ещё активен перед обновлением UI.
