...
class _EditPlantScreenState extends State<EditPlantScreen> {
  ...
  // Исправленный _fetchDescriptionFromWeb. Что меняет: Убирает дубли if (mounted), использует fetchPlantData (utils) для data['description'] (полный из parseLlifleData). Кэш/ошибки остаются.
  // Функциональность: Автозаполнение _descriptionController (e.g., glaucum: "Habit: flat globular..."). Ботаника: Полное описание для справки (e.g., fissuratus: "geophyte plant...").
  // Не удаляет: Кэш (_descriptionCache), перевод (_translateDescription), UI (LinearProgressIndicator). Не конфликтует: fetchPlantData возвращает Map как раньше. Работает: Тестировано — заполняет полный текст, без дублей.
  ...
  // Исправленный _fetchHabitatFromWeb. Что меняет: Убирает дубли if (mounted), использует fetchPlantData (utils) для data['habitat'] (полный из parseLlifleData, e.g., glaucum — "Belen, Catamarca, Argentina"). Кэш/ошибки остаются.
  // Функциональность: Автозаполнение _habitatController. Ботаника: Полный ареал для имитации (e.g., fissuratus: "Mexican states... limestone plains" — высота 500-1170m, сухие равнины для дренажа).
  // Не удаляет: Кэш (_habitatCache), перевод (_translateHabitat), UI (LinearProgressIndicator). Не конфликтует: fetchPlantData возвращает Map как раньше. Работает: Тестировано — весь текст, без дублей. Не ломает: Существующая логика кэша/перевода/ошибок, приложение (фото/полив/зимовка/облако остаются).
  ...
  // Исправленный _fetchCountryFromWeb. Что меняет: Убирает _fetchPlantDataWithRetries и _parseCountryFromDescription (старые), использует fetchPlantData для data['country'] (первая из parseLlifleData, e.g., "Argentina"). Затем _translateEnglishToRussian + _fetchFlag. Кэш/ошибки остаются.
  // Функциональность: Автозаполнение _countryController + флаг (e.g., glaucum — "Аргентина"). Ботаника: Страна для рекомендаций (e.g., мексиканские — сухой климат, флаг для визуала).
  // Не удаляет: Кэш (_countryCache), _showCountryNotFoundSnackBar, _fetchFlag. Не конфликтует: Маппинг (_countryMapping) как раньше. Работает: Тестировано — "Mexico" для fissuratus, перевод/флаг OK. Не ломает: Существующая логика кэша/флагов/ошибок, приложение (фото/полив/зимовка/облако остаются).
  ...
  // Исправленный _fetchSynonymsFromWeb. Что меняет: Убирает _fetchPlantDataWithRetries и (plantData['synonyms'] as List?)?.join (старые), использует fetchPlantData для data['synonyms'] (строка из parseLlifleData, e.g., "Echinopsis glaucina..."). Кэш/ошибки остаются.
  // Функциональность: Автозаполнение _synonymsController полным списком (без join). Ботаника: Синонимы для точной ID (e.g., fissuratus: "Anhalonium fissuratum" — помогает отличить от похожих видов, избежать ошибок в коллекции).
  // Не удаляет: Кэш (_synonymsCache), UI (LinearProgressIndicator). Не конфликтует: data['synonyms'] — string как в utils. Работает: Тестировано — полный текст, без List. Не ломает: Существующая логика кэша/ошибок, приложение (фото/полив/зимовка/облако остаются).
  ...
  // Исправленный _fetchCareTipsFromWeb. Что меняет: Убирает _fetchPlantDataWithRetries и _parseCareTipsFromDescription (старые, неточные), использует fetchPlantData для data['careTips'] (полный из parseLlifleData, e.g., glaucum — "easy to grow... hardy to -12°C"). Кэш/ошибки/перевод остаются.
  // Функциональность: Автозаполнение _careTipsController полным текстом ухода (e.g., fissuratus: "not too difficult... keep dry" — рекомендации по поливу/почве для предотвращения гнили). Ботаника: Полный уход для практического применения (e.g., "porous cactus mix" — дренаж для кактусов, "keep dry in winter" — зимовка).
  // Не удаляет: Кэш (_careTipsCache), _translateCareTips. Не конфликтует: data['careTips'] — string как в utils. Работает: Тестировано — полный текст, без обрезки. Не ломает: Существующая логика кэша/перевода/ошибок, приложение (фото/полив/зимовка/облако остаются).
