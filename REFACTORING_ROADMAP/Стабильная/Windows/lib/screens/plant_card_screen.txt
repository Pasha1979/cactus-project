...
// ←←← ЛЕНИВОЕ СКАЧИВАНИЕ ГЛАВНОГО ФОТО
            ...
            children: [
              ...
              Row(
                children: [
                  ...
                  if (plant.country != null) _buildCountryInfo(plant),
                ],
              ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          // Кнопка поиска взрослого фото с Llifle
          Center(
            child: ElevatedButton.icon(
              onPressed: () => _selectAdultImage(plant),
              icon: const Icon(Icons.search),
              label: const Text('Добавить фото с Llifle'),
  ...
  // Новый метод: сделать фото главным (только для своих фото)
  ...
  Future<void> _selectAdultImage(Plant plant) async {
    ...
    // === НОВОЕ: Фильтрация и очистка ссылок ===
    ...
    // Показываем диалог (теперь без искусственного ограничения в 6)
    if (mounted) {
      showDialog(
        context: context,
