...
|------|-------------|-----------|
| Memory leak — isolate не убивается | Средняя | `Isolate.kill()` при dispose, таймауты |
| Deadlock при коммуникации | Низкая | Таймауты на `SendPort.receive`, обработка ошибок |
| Сложность отладки | Высокая | Логирование через `debugPrint`, тесты на Windows |
| Падение производительности (оверхед) | Низкая | Бенчмарк: с isolate vs без, порог — 100 мс |
...
> **⚠️ Риски:** const-оптимизация может не сработать, RepaintBoundary может ухудшить производительность.
...
| Риск | Вероятность | Митигация |
|------|-------------|-----------|
| const-виджет не компилируется | Низкая | Проверить через `flutter analyze` после каждого |
| RepaintBoundary ухудшает FPS | Низкая | DevTools до/после, откатить если хуже |

---

### 2.6 Очистка deprecated кода (0.5 дня)

> **⚠️ Риски:** Можно удалить нужный код, сломать зависимости.
...
| Риск | Вероятность | Митигация |
|------|-------------|-----------|
| Удаление нужного кода | Средняя | `git diff` перед удалением, проверить `grep` по проекту |
| Поломка зависимостей | Низкая | `flutter analyze` после каждого удаления |
| Удаление platform-specific кода | Средняя | Проверить Android и Windows отдельно |
...
> **⚠️ Риски:** Новые правила линтера могут вызвать сотни предупреждений, сломать билд.
...
| Риск | Вероятность | Митигация |
|------|-------------|-----------|
| Сотни предупреждений | Высокая | Добавлять правила по одному, `flutter analyze` после каждого |
| Сломать билд Windows | Средняя | Проверять `flutter build windows` |
...
> **⚠️ Риски:** Lock + retry — сложная логика, может вызвать deadlock, потерю данных, бесконечный цикл.
>
...
| Риск | Вероятность | Митигация |
|------|-------------|-----------|
| Deadlock при Lock | Средняя | Таймауты на lock, `tryLock` с fallback |
| Потеря данных при валидации | Низкая | Не перезаписывать локальные данные пустым списком |
| Бесконечный цикл retry | Низкая | `maxSyncRetries = 3`, exponential backoff |
| Сломать существующую синхронизацию | Высокая | **Бэкап перед началом**, тестировать с реальным Яндекс Диском |
...
> **⚠️ Риски:** Может сломать синхронизацию Яндекс Диска, повредить фото, изменить пути.
>
...
| Риск | Вероятность | Митигация |
|------|-------------|-----------|
| Сломать синхронизацию Яндекс Диска | Высокая | **Бэкап перед началом**, тестировать с реальным фото, путь не менять |
| Повредить оригинал фото | Низкая | Сжимать КОПИЮ, оригинал оставить в галерее |
| Изменить путь к файлу | Средняя | Путь остаётся тем же, меняется только размер |
| Не работает на Windows | Низкая | `flutter_image_compress` уже проверен — собирается |
...
### 4.2b Полноценный экран настроек (1-1.5 дня) ✅ ЗАВЕРШЕН (2026-05-15)
...
✅ 4.2b.2 Создать детальные экраны для каждого раздела (11 экранов)
✅ 4.2b.3 Перенести функции из AppBar в соответствующие разделы:
   - ☁️ Подключение Яндекс.Диск (из AppBar) → `cloud_settings_screen.dart`
   - 🌤️ Погода — город и включение/выключение (из AppBar) → `weather_settings_screen.dart`
   - 🚪 Выход из аккаунта (из AppBar) → `system_settings_screen.dart`
✅ 4.2b.4 Сохранить в AppBar только часто используемые:
   - Поиск, Импорт Excel, Сохранить, Синхронизация, Настройки (⚙️)
✅ 4.2b.5 Создать `SettingsProvider` для хранения настроек
✅ 4.2b.6 Добавить `flutter analyze` ignore для deprecated Radio
✅ 4.2b.7 Исправить `use_build_context_synchronously` warnings
✅ 4.2b.8 Скопировать все изменения в Windows версию
...
**Проверка (10 пунктов):**
- ✅ **Пункт 1:** Все выполнено в полном объеме для Android и Windows
- ✅ **Пункт 2:** Все функции сохранены (сверка со "Стабильная" пройдена)
- ✅ **Пункт 3:** Платформенные различия сохранены (Windows _handleExit, Android)
- ✅ **Пункт 4-5:** Скрытые баги не обнаружены (mounted checks, dispose)
- ✅ **Пункт 6:** Логика архитектуры правильная (Clean Architecture, DI, Provider)
- ✅ **Пункт 7:** TODO запланированы в шаге 5.2 (6 TODO в настройках)
- ✅ **Пункт 8:** `flutter analyze` чистый (только info TODO)
- ✅ **Пункт 9:** Цели рефакторинга соблюдены (архитектура, производительность)
- ✅ **Пункт 10:** План на будущее логичен (шаг 5.2 запланирован)

**Все 10 пунктов проверки пройдены!**

---

### 5.2: Реализация TODO из настроек ✅ ЗАВЕРШЁН (2026-05-15)

**Цель:** Реализовать все TODO, оставленные в коде настроек при их создании, ДО финальной проверки.
...
ТОЛЬКО если приложение работает идеально (включая все TODO из 5.2) - разблокировать доступ к ROADMAP. Если есть проблемы - сначала исправить их, затем разблокировать.
